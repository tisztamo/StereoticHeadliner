...
async function sendMailerLiteCampaign(subject, htmlFilePath) {
  try {
    // Get MailerLite configuration from config object
    const apiKey = config.MAILERLITE_API_KEY;
    const groupId = config.MAILERLITE_GROUP_ID;
    const fromEmail = config.MAILERLITE_FROM_EMAIL;
    const fromName = config.MAILERLITE_FROM_NAME;
    
    // Check if all required configuration values are set
    if (!apiKey || !groupId || !fromEmail || !fromName) {
      console.error('\x1b[33m[MAILERLITE ERROR]\x1b[0m Missing required configuration values (MAILERLITE_API_KEY, MAILERLITE_GROUP_ID, MAILERLITE_FROM_EMAIL, MAILERLITE_FROM_NAME)');
      return false;
    }
